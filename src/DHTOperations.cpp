...
std::array<char, 32>
DHTOperations::putSes(std::array<char, 32> otherkey, uint32_t ip, uint16_t port,
		      lt::session *ses, bool relay)
{
  ...
  if(!relay)
      ...
      if(no->relayaddr.size() > 0)
	{
	  ...
	  ipstr = inet_ntop(AF_INET, &ip, &tmpbuf[0], socklen_t(tmpbuf.size()));
	  port = ntohs(port);
	  strm.str("");
	  strm.clear();
	  strm.imbue(loc);
  ...
  if(seq > 0)
      ...
      if(inet_ntop(AF_INET, &iploc, temp.data(), temp.size()))
	{
	  ...
	  if(itc != no->contacts.end())
	    {
	      no->getfrresmtx.lock();
	      auto it = std::find_if(no->getfrres.begin(), no->getfrres.end(),
				     [keyloc]
				     (auto &el) 
				       {
		  ...
		  if(ip != 0)
		    {
			  ...
			  no->maintblock.begin(), no->maintblock.end(), [keyloc]
			  (auto &el) 
			    {
			      return std::get<0>(el) == keyloc;
			    });
			...
			{
			  no->maintblock.push_back(std::make_tuple(key, bltm));
			}
  ...
  if(seq > 0)
    {
	  ...
	  << ntohs(port) << " seq=" << seq << std::endl;
      no->contmtx.lock();
      auto itc = std::find_if(no->contacts.begin(), no->contacts.end(), [keyloc]
      (auto &el) 
	{
	  return std::get<1>(el) == keyloc;
	});
...
DHTOperations::dhtThread(std::mutex *thrmtx)
{
      ...
      for(;;)
	{
	  ...
	  for(size_t i = 0; i < erasev.size(); i++)
	    {
	      ...
	      blockgetcall.erase(
		  std::remove_if(blockgetcall.begin(), blockgetcall.end(), [kk]
		  (auto &el) 
		    {
	  ...
	  for(size_t i = 0; i < geterr4.size(); i++)
	    {
	      ...
	      getv.erase(std::remove(getv.begin(), getv.end(), kk), getv.end());
	    }
	  ...
	  for(size_t i = 0; i < putv.size(); i++)
	      ...
	      if(it == putvinner.end())
		{
		  ...
		  ttup = std::make_tuple(otherkey, putkey, std::get<1>(putv[i]),
					 std::get<2>(putv[i]));
		  ...
		  if(itgvin != getvinner.end())
		    {
		      ...
		      auto itgeterr = std::find(geterr4.begin(), geterr4.end(),
						errk);
		      if(itgeterr == geterr4.end())
			{
			  auto itrcv4 = std::find(rcvdk4.begin(), rcvdk4.end(),
						  errk);
		  ...
		  if(itgvin6 != getvinner6.end())
		    {
		      ...
		      auto itgeterr6 = std::find(geterr6.begin(), geterr6.end(),
						 errk);
		      if(itgeterr6 == geterr6.end())
			{
			  auto itrcv6 = std::find(rcvdk6.begin(), rcvdk6.end(),
						  errk);
		      ...
		      else
			{
			  ...
			  if(n != std::string::npos)
			      ...
			      if(it != getvinner.end() && seq > 0)
				{
				  ...
				  std::locale loc("C");
				  strm.imbue(loc);
				  std::string portst = std::string(crv.begin(),
								   crv.end());
				  portst.erase(
				      0,
				      portst.find(":")
			  ...
			  if(n != std::string::npos)
			    {
			      std::array<char, 32> othk;
			      auto it = std::find_if(
				  getvinner6.begin(), getvinner6.end(), [keyarr]
				  (auto &el) 
				    {
				      return std::get<1>(el) == keyarr;
				    });
  ...
  f.write(sesbuf.data(), sesbuf.size());
  f.close();
  ses.abort();
  thrmtx->unlock();
}
  ...
  for(size_t i = 0; i < relayputinner->size(); i++)
    {
      ...
      auto itcont = std::find_if(no->contacts.begin(), no->contacts.end(), [key]
      (auto &el) 
	{
	  return std::get<1>(el) == key;
	});
