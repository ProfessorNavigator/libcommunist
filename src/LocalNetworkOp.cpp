...
#ifdef __linux
    setsockopt(sock6, IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP, &group, sizeof(group));
		...
		if(this->no->ownipv6 != "")
		  {
		    if(this->no->ipv6cont.size() < this->no->contacts.size())
		      {
			tmb = curtime;
			int ch = sendto(sock6, bindmsg6.data(), bindmsg6.size(),
					0, (struct sockaddr*) &sendipv6,
					sizeof(sendipv6));
			if(ch < 0)
			  {
#ifdef __linux
			...
			if(i == 0 && fd[i].revents == POLLRDNORM)
			  {
...
#endif
			    int n = recvfrom(fd[i].fd, buf.data(), buf.size(),
			    MSG_PEEK,
				...
				if(itcont != this->no->contacts.end())
				...
				else
				  {
				    ...
				    std::vector<char> tmpmsg;
				    tmpmsg.resize(INET_ADDRSTRLEN);
				    recvfrom(fd[i].fd, buf.data(), buf.size(),
					     0, (struct sockaddr*) &delfrom,
					     &sizefrom);
					...
					<< inet_ntop(AF_INET,
						     ...
						     tmpmsg.data(),
						     tmpmsg.size()) << ":"
					<< ntohs(delfrom.sin_port) << std::endl;
				  }
				this->no->contmtx.unlock();
			      }
			    else
				...
				if(itcont != this->no->contacts.end())
				  {
					...
					std::copy(sndstr.begin(), sndstr.end(),
						  std::back_inserter(msg));
					sockaddr_in6 repadr =
					  { };
