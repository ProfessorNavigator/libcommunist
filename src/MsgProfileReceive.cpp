			   ...
			   int sockipv4, std::vector<char> &buf, bool relay)
{
  ...
  int chprexists = 0;
  no->contmtx.lock();
  auto contit = std::find_if(no->contacts.begin(), no->contacts.end(), [keyarr]
  (auto &el) 
    {
			   ...
			   std::vector<char> &buf)
{
  ...
  no->msghashmtx.lock();
  auto itmh = std::find_if(no->msghash.begin(), no->msghash.end(), [keyarr, tm]
  (auto &el) 
    {
      if (std::get<0>(el) == keyarr && std::get<1>(el) == tm)
	{
			   ...
			   std::vector<char> &buf)
{
  ...
  if(itmph != no->msgparthash.end())
      ...
      if(itmpr == no->msgpartrcv.end())
	{
	  ...
	  no->msgpartrcv.push_back(std::make_tuple(keyarr, tm, partnum, part));
	}
			   ...
			   int sockipv4, std::vector<char> &buf, bool relay)
{
  ...
  no->msgpartrcvmtx.lock();
  no->msgpartrcv.erase(
      std::remove_if(no->msgpartrcv.begin(), no->msgpartrcv.end(), [keyarr, tm]
      (auto &el) 
	{
	  if (std::get<0>(el) == keyarr && std::get<1>(el) == tm)
	    {
			   ...
			   std::vector<char> &buf)
{
  ...
  no->msgpartbufmtx.lock();
  no->msgpartbuf.erase(
      std::remove_if(no->msgpartbuf.begin(), no->msgpartbuf.end(), [keyarr, tm]
      (auto &el) 
	{
	  if (std::get<0>(el) == keyarr && std::get<1>(el) == tm)
	    {
	  ...
	  if(itmh != no->msghash.end())
		  ...
		  && msgsz == uint64_t(std::filesystem::file_size(filepath)))
		{
		  ...
		  if(std::filesystem::exists(outpath))
			...
			{
			  no->messageReceived(lt::aux::to_hex(keyarr), outpath);
			}
		      af.updateMsgLog(no->Home_Path, no->Username, no->Password,
				      keyarr, outpath.u8string(), no->contacts);
		      result = 1;
		    }
		}
	    }
