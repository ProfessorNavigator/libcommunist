			      ...
			      sockaddr_in6 *from6, bool relay)
{
  ...
  AuxFuncNet af;
  no->contmtx.lock();
  auto contit = std::find_if(no->contacts.begin(), no->contacts.end(), [chkey]
  (auto &el) 
    {
  ...
  if(itfpb != no->filepartbuf.end())
      ...
      else
	{
	  if(no->filepartsendsig)
	    {
	      no->filepartsendsig(lt::aux::to_hex(keyarr), std::get<2>(*itfpb),
  ...
  if(itfhv != no->filehashvect.end())
    {
	  ...
	  no->fileparthash.end());
      no->fileparthash.push_back(std::make_tuple(keyarr, tint, partnum, hash));
			...
			int sockipv, sockaddr_in *from, sockaddr_in6 *from6)
{
  ...
  for(;;)
    {
      auto itfpr = std::find_if(no->filepartrcv.begin(), no->filepartrcv.end(),
				[key]
				(auto &el) 
				  {
      ...
      if(itfpr != no->filepartrcv.end())
	{
	  ...
	  if(itfprl != no->filepartrlog.end())
	      ...
	      else
		{
		  ...
		  std::copy(part.begin(), part.end(), std::back_inserter(tv));
		  std::get<2>(*itcpv) = tv;
		}
	      no->currentpartmtx.unlock();
			...
			int sockipv, sockaddr_in *from, sockaddr_in6 *from6)
{
  ...
  for(;;)
    {
      auto itfpe = std::find_if(no->filepartend.begin(), no->filepartend.end(),
				[key]
				(auto &el) 
				  {
				    return std::get<0>(el) == key;
