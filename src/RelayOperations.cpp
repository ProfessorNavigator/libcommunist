      ...
      if (relsz > 0)
	{
	  ...
	  for (size_t i = 0; i < relayaddr->size (); i++)
	      ...
	      if (curtime - chtm > 1200)
		{
		  ...
		  std::get<0> (tmptup) = ip;
		  std::get<1> (tmptup) = port;
		  auto itfr = std::find (fraddrs.begin (), fraddrs.end (),
					 tmptup);
		  if (itfr == fraddrs.end ())
		    {
		      fraddrs.push_back (tmptup);
	  ...
	  for (size_t i = 0; i < fraddrs.size (); i++)
		...
		{ };
	      ...
	      std::copy (msgstr.begin (), msgstr.end (),
	  ...
	  if (fdsl[0].revents == POLLRDNORM)
		...
		{ };
		  ...
		  << inet_ntop (AF_INET, &rcvdaddr.sin_addr.s_addr, &tmpv[0],
				tmpv.size ()) << ":"
		  << ntohs (rcvdaddr.sin_port) << std::endl;
	      std::mutex *thrmtxl = new std::mutex;
	      thrmtxl->lock ();
	      threadvectmtx->lock ();
	      threadvect->push_back (
		  std::make_tuple (thrmtxl, "relaySrvThread"));
	      threadvectmtx->unlock ();
	  ...
	  if (type == "req")
	    {
	      ...
	      std::copy (msgstr.begin (), msgstr.end (),
			 std::back_inserter (msg));
	      std::copy (ownkey.begin (), ownkey.end (),
			 std::back_inserter (msg));
	  ...
	  if (itkv != keyvect.end ())
			  ...
			  if (itrv != relayvect.end ())
			    {
			      ...
			      std::copy (keyfm.begin (), keyfm.end (),
					 std::back_inserter (msg));
			      std::array<char, 64> signtos = std::get<3> (
				  *itrv);
	      ...
	      if (msgtype == "RM" && lbuf.size () > 130)
		{
		  ...
		  std::copy (lbuf.begin () + 34, lbuf.begin () + 66,
			     keyfm.data ());
		  std::array<char, 64> sign;
		  std::copy (lbuf.begin () + 66, lbuf.begin () + 130,
			     sign.data ());
		  lt::dht::public_key pk;
		  pk.bytes = keyfm;
		  lt::dht::signature sig;
      ...
      else
	{
	  ...
	  if (establishConnect (sock, &restup))
	      ...
	      for (size_t j = 0; j < lmsgsbuf.size (); j++)
		{
		  ...
		  std::string passwd = oaf.genFriendPasswd (
		      othkey, std::get<1> (restup));
		  AuxFuncNet afn;
    ...
    int sock, std::tuple<int, std::array<char, 32>, std::array<char, 32>> *res)
{
  ...
  if (ssz > 0)
      ...
      if (msgbuf.size () > 0)
	{
	  buf = msgbuf[0];
	  std::string msgtype;
	  std::copy (buf.begin (), buf.begin () + 3,
		     std::back_inserter (msgtype));
	  if (msgtype == "rep")
	    {
	      sockaddr_in addr =
